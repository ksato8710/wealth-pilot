import { NextResponse } from "next/server";
import { prisma } from "@/lib/server/prisma";
import { getAllHoldings } from "@/lib/server/services/holdings-service";
import {
  getAssetAllocation,
  getSectorAllocation,
} from "@/lib/server/services/portfolio-service";

const DEFAULT_USER_ID = "default-user";

async function ensureUser() {
  const user = await prisma.user.findUnique({
    where: { id: DEFAULT_USER_ID },
  });
  if (!user) {
    await prisma.user.create({
      data: {
        id: DEFAULT_USER_ID,
        name: "Default User",
        email: "user@wealthpilot.local",
      },
    });
  }
}

export async function GET() {
  try {
    await ensureUser();

    const holdings = await getAllHoldings(DEFAULT_USER_ID);
    const assetAllocation = getAssetAllocation(holdings);
    const sectorAllocation = getSectorAllocation(holdings);

    return NextResponse.json({ assetAllocation, sectorAllocation });
  } catch (error) {
    console.error("GET /api/portfolio/allocation error:", error);
    return NextResponse.json(
      { error: "Failed to fetch portfolio allocation" },
      { status: 500 }
    );
  }
}
